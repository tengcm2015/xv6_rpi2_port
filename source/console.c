...
uint initframebuf(uint width, uint height, uint depth)
{
    ...
    fbinfo.fbp = 0;
    fbinfo.fbs = 0;
    writemailbox(MB0_FRAMEBUFFER, (uint)&fbinfo);
    return readmailbox(MB0_FRAMEBUFFER);
}
...
void drawpixel(uint x, uint y)
{
    ...
    if (x >= framewidth || y >= frameheight)
        return;
    addr = (uint16 *) fbinfo.fbp;
    // addr = (uint16 *) ((FBI *)FrameBufferInfo)->fbp;
    addr += y*1024 + x;
    *addr = gpucolour;
    return;
}
...
void drawcharacter(uint8 c, uint x, uint y)
{
    uint8 *faddr;
    uint8 row, bit, bits;
    uint tv;
...
gpuputc(uint c)
{
    if(c=='\n'){
        ...
        if(cursor_y >= frameheight) {
            memmove( (uint8 *)fbinfo.fbp
                   , (uint8 *)fbinfo.fbp + framewidth * fontheight * 2
...
{
    static uint8 digits[] = "0123456789abcdef";
    uint8 buf[16];
    int i;
    uint x, y, b;
